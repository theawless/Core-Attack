...
function Awake () {
	...
	for (var moveAnimation : MoveAnimation in moveAnimations) {
		...
		animationComponent[moveAnimation.clip.name].speed = 1.5;
...
function LateUpdate () {
	...
	if (idle < 1) {
		...
		if(ground)
		var lowerBodyDeltaAngleTarget : float = Mathf.DeltaAngle (HorizontalAngle (tr.rotation * animatedLocalVelocity), HorizontalAngle (velocity));
		...
		lowerBodyDeltaAngle = Mathf.DeltaAngle (HorizontalAngle (tr.forward), HorizontalAngle (lowerBodyForward));
